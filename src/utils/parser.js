...
function parse(fileName, gltf, options = {}) {
    ...
    if (
        ...
        if (
          ...
          if (first.children)
            first.children.forEach(
              (child) => (children += printAngularThree(child, true, false)),
            );
          return children;
        }
      }
      ...
      if (
        ...
        if (
          ...
          children = printAngularThree(first, true, false);
          return children;
        }
      }
  ...
  let hasArgs = false;
  function printAngularThree(obj, silent = false, first = true) {
    let result = "";
    let children = "";
    ...
    if (obj.__removed && obj.children.length) {
      obj.children.forEach(
        (child) => (result += printAngularThree(child, silent, false)),
      );
      return result;
    }
    ...
    if (obj.children)
      obj.children.forEach(
        (child) => (children += printAngularThree(child, silent, false)),
      );
  ...
  if (!options.keepgroups) {
    ...
    printAngularThree(gltf.scene, false, true);
  ...
  const scene = printAngularThree(gltf.scene, false, true);
  ...
  const imports = `
        ...
        import { Component, ChangeDetectionStrategy, CUSTOM_ELEMENTS_SCHEMA, Signal, input, computed, viewChild, ElementRef, effect${hasAnimations ? ", model" : ""} } from '@angular/core';
...
export class Model {
    ...
    ${hasAnimations ? "animations = model<ActionName>();" : ""}
